...
if (
  isNull(script.customCategory) ||
  script.customCategory.length == 0 ||
  script.customCategory == "DEFAULT"
) {
...
global.Heartbeat.api.add(
  script,
  script.customCategory ? script.customCategory : "DEFAULT"
);
eval(
...
var createStateCallback = function (key) {
  ...
  script.api["subscribeOn" + upperKey + "Deep"]("default", function () {
    for (var i = script.subMachines.length - 1; i >= 0; i--) {
      if (isNull(script.subMachines[i])) continue;
...
var translateVariables = function (objScript, inputStr) {
  ...
  for (let match of matches) {
    ...
    if (
      variableOwner != null &&
      variableOwner[variableName].toString().length > 0
    ) {
...
var processStates = function () {
  ...
  for (var i = 0; i < script.states.length; i++) {
    ...
    var regex = /(\w+)\(((?:[^)(]+|\((?:[^)(]+|\([^)(]*\))*\))*)\)|(\w+)/g;
    var functions = [];
    var result = sequence.replace(
      regex,
      function (match, functionName, args, functionNameNoParens) {
        if (functionName) {
          if (args != null) {
            const regex2 = /,(?![^(]*\))/g;
            ...
            for (var a = 0; a < _args.length; a++) {
              ...
              if (_args[a].includes("(")) {
                ...
                if (fnArgs != null && fnArgs.length > 1) {
                  args +=
                    (script.api[fn] ? "script.api." : "global.Shortcuts.api.") +
                    fn +
                    "(script," +
                    fnArgs.replace(")", "") +
                    ")" +
          ...
          return functions.push(
            (script.api[functionNameNoParens]
              ? "script.api."
              : "global.Shortcuts.api.") +
              functionNameNoParens +
              "(script)"
          );
        }
      }
...
var runState = function () {
  ...
  if (script.events.length == 0) return;
  ...
  if (!script.focus && eventC == 0) script.api.onStateComplete();
};
...
var gatherEmbeddedStateMachines = function () {
  ...
  var totalChildren = function (a) {
    ...
    for (var i = 0; i < a.length; i++) {
      if (!isNull(a[i])) c += a[i].getChildrenCount();
    }
    return c;
  };
  ...
  while (first || totalChildren(children) > 0 || last) {
    for (var i = children.length - 1; i >= 0; i--) {
      ...
      if (children.length == 0) last = false;
    }
  }
};
...
var fast = function (objScript) {
  ...
  while (
    ...
    _state == currState &&
    !objScript.focus &&
    script.events[0].split("(")[0] != "global.Shortcuts.api.or" &&
    script.events[0].split("(")[0] != "global.Shortcuts.api.delay" &&
    script.events[0].split("(")[0] != "script.api.unfast" &&
    script.events[0].split("(")[0] != "global.Shortcuts.api.focus" &&
    !script.events[0].split(".api.")[1].startsWith("on") &&
    !script.events[0].split(".api.")[1].startsWith("do")
  ) {
    runState();
  }
  script.api.addUpdate("heart", runState);
...
script.go = function (stateName) {
  script.api.go(script, stateName);
};

script.api.gatherEmbeddedStateMachines = gatherEmbeddedStateMachines;

var initImmediately =
  script.states.length > 0 &&
  (script.states[0].indexOf(":") > -1
    ? script.states[0].split(":")[1]
    : script.states[0]
  ).startsWith("fast");
